...
<form action="<?php echo Route::_('index.php' . ($auth == 'TLS' ? '?idp=cns' : ''), true, $params->get('usesecure')); ?>" method="post" id="cie-login-form"
	...
	<div class="u-margin-bottom-m">
		<p class="u-text-p"><?php echo Text::_('MOD_LOGIN_CIE_LOGIN_TEXT'); ?></p>
	</div>
	...
	<div class="u-flex u-margin-bottom-m">
		<div class="u-layoutCenter">
			...
			<?php else : // if ($auth == 'TLS') : ?>
				<a href="#" class="u-text-xs js-fr-dialogmodal-open" aria-controls="modal-entra-con-cie">
				...
				<?php echo HTMLHelper::_('bootstrap.renderModal', 'modal-entra-con-cie', 
					...
					'footer' => '<div class="u-flex u-margin-bottom-m"><div class="u-layoutCenter">' . 
						'<a href="#" class="Button Button--danger u-text-xs js-fr-dialogmodal-close">' . Text::_('JLIB_HTML_BEHAVIOR_CLOSE') . '</a> ' .
						'<button type="submit" class="Button Button--default u-text-xs">' . Text::_('JLOGIN') . '</button></div></div>'],
						'<div class="u-margin-bottom-m"><p class="u-text-p">' . Text::_('MOD_LOGIN_CIE_POPUP_TEXT') . '</p></div>'); ?>		
			<?php endif; ?>
		</div>
	</div>
